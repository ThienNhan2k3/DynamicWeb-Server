function showEditAdTypeModal(btn) {
...
if (confirmEditButton) {
  confirmEditButton.addEventListener("click", async (e) => {
    ...
    try {
      const res = await fetch("/department/adTypeManagement", {
        method: "PUT",
        headers: {
...
function showDeleteAdTypeModal(btn) {
  const id = btn.dataset.id;
  console.log(id);
...
if (confirmDeleteButton) {
  confirmDeleteButton.addEventListener("click", async (e) => {
    e.preventDefault();
    const data = {
      adTypeId: document.getElementById("idDeleteModal").value,
    };
    console.log(data);
    const res = await fetch("/department/adTypeManagement", {
      method: "DELETE",
