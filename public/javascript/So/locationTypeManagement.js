function showEditLocationTypeModal(btn) {
...
if (confirmEditButton) {
  confirmEditButton.addEventListener("click", async (e) => {
    ...
    try {
      const res = await fetch("/department/locationTypeManagement", {
        method: "PUT",
        headers: {
...
function showDeleteLocationTypeModal(btn) {
  const id = btn.dataset.id;
  console.log(id);
...
if (confirmDeleteButton) {
  confirmDeleteButton.addEventListener("click", async (e) => {
    e.preventDefault();
    const data = {
      locationTypeId: document.getElementById("idDeleteModal").value,
    };
    console.log(data);
    const res = await fetch("/department/locationTypeManagement", {
      method: "DELETE",
